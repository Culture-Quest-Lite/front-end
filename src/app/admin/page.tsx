...
export default function AdminDashboardPage() {
  ...
  return (
    <div className="space-y-6">
          ...
          <div className="h-56">
            <ResponsiveContainer width="100%" height="100%">
              ...
              >
                ...
                <XAxis dataKey="d" stroke="rgb(var(--muted-foreground))" fontSize={11} tickLine={false} axisLine={false} />
                <YAxis stroke="rgb(var(--muted-foreground))" fontSize={11} tickLine={false} axisLine={false} />
                <Tooltip
                  contentStyle={{
                ...
                />
                <Area type="monotone" dataKey="v" stroke="rgb(var(--primary))" strokeWidth={2.5} fill="url(#g1)" />
              </AreaChart>
            </ResponsiveContainer>
          </div>
        </div>
          ...
          <div className="h-56">
            <ResponsiveContainer width="100%" height="100%">
              ...
              >
                ...
                />
                <Line type="monotone" dataKey="u" stroke="rgb(var(--accent))" strokeWidth={2.5} dot={{ r: 3 }} />
              </LineChart>
            </ResponsiveContainer>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-4">
        <div className="rounded-2xl bg-white p-4 shadow-sm xl:col-span-2">
          <div className="flex items-center justify-between mb-3">
            ...
            <Link href="/analytics" className="text-xs text-primary inline-flex items-center gap-0.5">
              Xem tất cả <ArrowUpRight className="w-3 h-3" />
            </Link>
          </div>
          <div className="h-56">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={routeEngagement} margin={{ left: -16, right: 8, top: 8, bottom: 0 }}>
                <CartesianGrid strokeDasharray="3 3" stroke="var(--border)" vertical={false} />
                <XAxis dataKey="r" stroke="rgb(var(--muted-foreground))" fontSize={11} tickLine={false} axisLine={false} />
                <YAxis stroke="rgb(var(--muted-foreground))" fontSize={11} tickLine={false} axisLine={false} />
                <Tooltip
                  contentStyle={{
                    background: "rgb(var(--popover))",
                ...
                />
                <Bar dataKey="views" fill="rgb(var(--chart-3))" radius={[6, 6, 0, 0]} />
                <Bar dataKey="completes" fill="rgb(var(--primary))" radius={[6, 6, 0, 0]} />
              </BarChart>
            </ResponsiveContainer>
          </div>
        </div>
          ...
          <ul className="space-y-2.5">
            {approvals.map((a) => (
              <li key={a.id} className="flex items-center gap-3">
                <img src={a.thumbnail} alt="" className="w-10 h-10 rounded-lg object-cover" />
                <div className="min-w-0 flex-1">
                  <div className="text-sm font-medium truncate">{a.title}</div>
                  <div className="mt-1 flex flex-wrap items-center gap-1.5 text-[11px] text-slate-500">
                    <span className="rounded-md bg-slate-100 px-1.5 py-0.5">{a.type}</span>
                    <span>· {a.curator}</span>
        ...
        <div className="rounded-2xl bg-white p-4 shadow-sm xl:col-span-2">
          ...
          <div className="space-y-3">
            {users.slice(0, 4).map((user) => (
              <div key={user.id} className="flex items-center gap-3 rounded-2xl border border-slate-200/70 bg-slate-50 p-3">
        ...
        <div className="rounded-2xl bg-white p-4 shadow-sm">
          ...
          <div className="space-y-3">
            {reports.slice(0, 3).map((report) => (
              <div key={report.id} className="rounded-2xl border border-slate-200/70 p-3">
                <div className="flex items-center justify-between gap-3">
                  <div>
                    <div className="text-sm font-medium">{report.type}</div>
                    <p className="text-xs text-slate-500">{report.target}</p>
                  </div>
                  <span className="text-[11px] font-semibold text-slate-500">{new Date(report.at).toLocaleDateString("vi-VN")}</span>
                </div>
      ...
      <div className="rounded-2xl bg-white p-4 shadow-sm">
        <div className="flex items-center justify-between mb-3">
          <div className="text-sm font-semibold">Hoạt động quản trị gần đây</div>
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          {audit.slice(0, 4).map((entry) => (
            <div key={entry.id} className="rounded-2xl border border-slate-200/70 p-3">
