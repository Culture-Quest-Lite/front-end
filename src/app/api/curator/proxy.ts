...
export async function forwardCuratorRequest(
  request: NextRequest,
  method: "GET" | "POST" | "PUT" | "PATCH" | "DELETE",
  curatorPath: string[],
) {
  const backendUrl = buildBackendUrl(request, curatorPath);
...
function buildBackendUrl(request: NextRequest, curatorPath: string[]) {
  const normalizedBaseUrl = BACKEND_API_BASE_URL.endsWith("/")
    ? BACKEND_API_BASE_URL.slice(0, -1)
    : BACKEND_API_BASE_URL;
  const joinedPath = curatorPath.length > 0 ? `/${curatorPath.join("/")}` : "";

  return `${normalizedBaseUrl}/api/curator${joinedPath}${request.nextUrl.search}`;
}
