...
type VoucherMethod = "GET" | "POST";

async function forwardVoucherRequest(request: NextRequest, method: VoucherMethod) {
...
function buildBackendUrl(request: NextRequest) {
  ...
  const requestPath = request.nextUrl.pathname.replace(/^\/api\/vouchers/, "");

  return `${normalizedBaseUrl}/api/vouchers${requestPath}${request.nextUrl.search}`;
}
...
export async function GET(request: NextRequest) {
  return forwardVoucherRequest(request, "GET");
}

export async function POST(request: NextRequest) {
  return forwardVoucherRequest(request, "POST");
}
